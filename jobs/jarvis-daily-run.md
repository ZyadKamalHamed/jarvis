...
3. **Study.** Run `python3 bin/study.py refresh` (FSRS active recall). The refresh auto-captures any vault note written or edited in the last 36 hours into the rotation (no tag needed; #review remains the explicit opt-in) and honours per-card `startDate` staggering, so imported decks introduce themselves one card a day. Read the resulting `data/study.json` stats: `dueCount` and `sessionMin` (5 to 20 minutes) size today's recall load. Then run `node bin/study-exgen.mjs --due --quiet` so every due card has its interactive exercises (multiple choice, flashcards, code task) before he presses PLAY; generation failures are fine, those cards fall back to the classic reveal. If the vault is missing, mark the pipeline degraded in `system.json` instead of failing.
