...
3. **Study.** SRS trial since 26 Jul 2026: reviews live in Anki (synced from the vault Flashcards folder by the Yanki plugin) and the Obsidian Spaced Repetition plugin (inline cards in notes tagged #flashcards). Two jobs, in this order:
   - **Draft flashcards for fresh notes.** Run `python3 bin/flashcard-candidates.py`. For each candidate (cap 5 per morning, newest first): read the note and APPEND a `## Flashcards` section at the end, then the subject tag on its own line, then 6 to 10 inline cards, one per line, formatted question, two colons, answer. Tag by subject folder: 31268 Web Systems -> `#flashcards/web-systems`, 41092 Network Fundamentals -> `#flashcards/networks`, 41080 Theory of Computing Science -> `#flashcards/tcs`, 57304 Ethics -> `#flashcards/ethics`, anything else -> `#flashcards/general`. Card rules are binding: atomic (one fact or skill each), fronts answerable in under 30 seconds, base every card strictly on what the note says, correct any obvious factual or code error on the answer side rather than memorialising it (note the correction in brackets), no em dashes, Australian English, and the `::` separator must appear exactly once per card line. APPEND ONLY: never edit, reword or delete his prose, never touch notes that already carry `#flashcards`, and skip a note rather than guess if its content is too thin to card honestly. The briefing's study line then says cards were drafted and for which notes so he can glance before first review.
   - **Counts.** Run `python3 bin/anki-due.py --write`; it prints a JSON summary (per-deck Anki due/new counts, Obsidian plugin due/new counts, `careerOnly`) and writes `data/study.json` with an empty queue so the HUD shows the trial note instead of a PLAY session. Do NOT run `bin/study.py refresh` or `bin/study-exgen.mjs`; the legacy FSRS pipeline is retired for the trial. If the script reports `"source": "unavailable"`, mark the study pipeline degraded in `system.json` and carry on. The trial decision is due Sun 9 Aug 2026: from 7 Aug, add one board task reminding him to pick Anki or the Obsidian plugin.
...
- READ ONLY outside this repo: AIOS is read-only, the web is read-only (HEAD checks and GET pages; never log in, submit, apply or accept anything). Two vault exceptions, both append-only: the daily log via `bin/obsidian-note.mjs`, and `## Flashcards` sections appended to candidate study notes per phase 1 step 3. Nothing else in the vault may be created, edited or deleted.
