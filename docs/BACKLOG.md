...
2. **Sonder prep pack** (before 22 Jul). Career-analyst subagent builds role research, likely questions, story mapping into a briefing section plus a drafts/ prep sheet.
3. **Calendar access grant follow-through.** The module shipped 10 Jul but macOS would not present the permission prompt from the build sandbox; if the launchd daily run also cannot get it, walk Zyad through the one-command grant in the setup note, then delete this item.
4. **Health workout parsing.** `/api/health` ingests weight and nutrition; extend to the `workouts` array (last session name, weekly count) when real pushes start arriving.
