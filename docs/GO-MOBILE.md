...
5. **Always-on server and agents:** `mkdir -p ~/Library/LaunchAgents && cp deploy/com.zyad.jarvis.plist deploy/com.zyad.jarvis.daily.plist deploy/com.zyad.jarvis.evolve.plist ~/Library/LaunchAgents/` then `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/<each>.plist`. Check the node path inside com.zyad.jarvis.plist matches `which node` on that machine, and the hardcoded /Users/Zyad paths if the username differs.
