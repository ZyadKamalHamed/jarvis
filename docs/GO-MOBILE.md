...
5. **Always-on server:** `mkdir -p ~/Library/LaunchAgents && cp deploy/com.zyad.jarvis.plist ~/Library/LaunchAgents/ && launchctl load ~/Library/LaunchAgents/com.zyad.jarvis.plist`. Check the node path inside the plist matches `which node` on that machine.
...
8. **Scheduled agents:** in the Claude desktop app ON THE HOME LAPTOP, create both tasks (New task): "Read and follow /Users/Zyad/Documents/Claude/Scheduled/jarvis-daily-run/SKILL.md exactly." scheduled daily 7:30am, and the same for jarvis-evolve at 9:30pm. Copy the two SKILL.md folders from this machine (same Taildrop trick) or recreate them from the repo's jobs/ specs.
