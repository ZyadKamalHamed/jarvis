...
`/api/data` additionally serves `wins` (last 50), `metrics` (last 14 lines of metrics.jsonl, career pipeline key removed in work mode), `evolution` (evolution.json; its `summary` must always be work-safe), `proposals` (career-flagged ones stripped in work mode) and `agents` (the agent-deck roster: built-ins hardcoded in server.mjs plus `.claude/agents/*.md` frontmatter parsed live, including `role`, optional `career` and optional `workDescription` keys; work mode drops career agents, swaps in workDescription, and sheds both flag keys). The HUD renders the roster as the hologram Agent deck panel; each figure name comes from the server's `figure` field.
